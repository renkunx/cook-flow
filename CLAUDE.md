...
1. 抓取后的数据使用大语言模型进行处理，统一支持大语言模型文本翻译，包括名称、步骤、描述；如果抓取的数据没有菜品描述，则使用大语言模型进行生成（主要介绍菜品和菜品的营养搭配）；步骤的数据如果有图片，则用 ai 识别图片，将图片内容转换为文字，步骤里的食材需要提取出来。
2. 基于菜品的食材，需要创建属性字段，菜品包含的（脂肪、蛋白质、卡路里和碳水）含量